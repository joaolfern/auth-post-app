...
function ContextProvider({ children }) {
  ...
  const parseMessage = (message) => {
    ...
    const translatedMessage = translateMessage(message.split(`"${subject}"`).slice(-1)[0])
    console.log(message, subject, translatedSubject, translatedMessage)

    return translatedSubject + translatedMessage

    message.replace(`"${subject}"`, translatedSubject)
    switch (message) {
      case '"display_name" is not allowed to be empty':
